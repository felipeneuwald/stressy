...
- The Docker image is now `FROM scratch` rather than Alpine, and runs as UID/GID `65532` instead of root. A root-only image cannot be admitted into a Kubernetes namespace enforcing the `restricted` Pod Security Standard, which for a tool people deploy into clusters was a scheduling blocker rather than an exploitation risk. Going to `scratch` also retires the base image as a source of CVEs entirely: images are only rebuilt on a release tag, so any distribution base accumulated advisories between releases with nothing to rebuild it. The binary is built `CGO_ENABLED=0` and needs no userland. The trade-off is that there is no shell, so `docker exec` and `kubectl exec` into a running container no longer work (#21)
